...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011349 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2013011349 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2013011349 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011349
    //LAB8:EXERCISE2 2013011349  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2013011349  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(3.1) read raw data content in file and resolve elfhdr
    if((ret=load_icode_read(fd,elf,sizeof(struct elfhdr),0))!=0){			//difference;
    ...
    for(phnum=0;phnum<elf->e_phnum;phnum++) {
        ...
        if((ret=load_icode_read(fd,ph,sizeof(struct proghdr),phoff))!=0){	//difference;
        ...
        for(;start<end;){
            ...
            if((ret=load_icode_read(fd,page2kva(page)+off,size,offset))!=0){	//difference;
                goto bad_cleanup_mmap;
            }
